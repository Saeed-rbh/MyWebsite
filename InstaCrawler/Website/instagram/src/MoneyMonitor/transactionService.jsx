...
const groupTransactionsByMonth = (transactions) => {
  ...
  const completeGroupedTransactions = {};
  const currentDate = new Date();
  const oldestTransactionDate = new Date(transactions[0].Timestamp);
  let currentMonth = currentDate.getMonth();
  let currentYear = currentDate.getFullYear();

  while (
    currentYear > oldestTransactionDate.getFullYear() ||
    (currentYear === oldestTransactionDate.getFullYear() &&
      currentMonth >= oldestTransactionDate.getMonth())
  ) {
    const month = String(currentMonth + 1).padStart(2, "0");
    const key = `${currentYear}-${month}`;

    if (!groupedTransactions[key]) {
      completeGroupedTransactions[key] = {
        transactions: [],
        totalSpending: 0,
        totalIncome: 0,
        totalSaving: 0,
      ...
      completeGroupedTransactions[key] = groupedTransactions[key];
    }
  ...
  // Calculate percentage changes for each month
  let previousNetTotal = null;
  Object.keys(completeGroupedTransactions)
    .sort()
    .forEach((key, index) => {
      const currentNetTotal = completeGroupedTransactions[key].netTotal;
      if (index > 0 && previousNetTotal !== null) {
        const percentageChange =
          ((currentNetTotal - previousNetTotal) / Math.abs(previousNetTotal)) *
          100;
        completeGroupedTransactions[key].percentageChange = parseInt(
  ...
  return completeGroupedTransactions;
};
