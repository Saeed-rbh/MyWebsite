...
function Qualifications() {
  ...
  // Adjusted Top Calculation using useCallback
  const calculateAdjustedTop = useCallback(() => {
    const ModifyTop = 100;
    ...
    const topCheck =
      ...
      (isActive ? activeHeight : notActiveHeight) -
      1.5 * ModifyTop;

    if (isActive) {
      if (topCheck < 0) {
        newAdjustedTop = newAdjustedTop + topCheck;
      } else if (activeHeight > viewportHeight) {
        newAdjustedTop =
  ...
  // Use Effect to Update Top
  useEffect(() => {
    setAdjustedTop(calculateAdjustedTop());
  }, [calculateAdjustedTop]);
